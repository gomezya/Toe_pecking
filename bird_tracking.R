...
# Strange 2042 years stamp that need to change

data <- data %>% 
  mutate(V1 = recode(V1, "24.10.2048" = "24.10.2021"))
str(data)
...
data[, Tier := revalue(as.character(V4), tierMapping)]

# Separate way of recoding names if want to keep original  
data <- data %>% 
  mutate(Zone = recode(V4, !!!zoneMapping))

data <- data %>% 
  mutate(Side = recode(Zone, !!!sideMapping))

data <- data %>% 
  mutate(Tier = recode(V4, !!!tierMapping))
